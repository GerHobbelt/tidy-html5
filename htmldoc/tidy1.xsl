<?xml version="1.0"?>
<!--
    For generating the `tidy5.1` man page from the
    output of `tidy5 -xml-help` and `tidy5 -xml-config`
...
<!--
    ...
    command line to the XSLT processor, currently "tidy5-help.xml".
    For the detailed config options section however, the template
    match is to the file "tidy5-config.xml".  This is captured in
    the $CONFIG variable, declared here:
-->

<xsl:variable name="CONFIG" select="document('tidy5-config.xml')"/>
...
.ad l
.SH NAME
\fBtidy5\fR - check, correct, and pretty-print HTML(5) files
.SH SYNOPSIS
\fBtidy5\fR [option ...] [file ...] [option ...] [file ...]
.SH DESCRIPTION
...
.LP
.in 1i
\fBtidy5 -f errs.txt -imu foo.html\fR
.SH ENVIRONMENT
.TP
.B HTML_TIDY
Name of the default configuration file.  This should be an absolute path, since you will probably invoke \fBtidy5\fR from different directories.  The value of HTML_TIDY will be parsed after the compiled-in default (defined with -DTIDY_CONFIG_FILE), but before any of the files specified using \fB-config\fR.
.SH "EXIT STATUS"
.IP 0
...
This section describes the Detailed (i.e., "expanded") Options, which may be specified by preceding each option with \fB--\fR at the command line, followed by its desired value, OR by placing the options and values in a configuration file, and telling tidy5 to read that file with the \fB-config\fR standard option.
.SH SYNOPSIS
\fBtidy5 --\fR\fIoption1 \fRvalue1 \fB--\fIoption2 \fRvalue2 [standard options ...]
.br
\fBtidy5 -config \fIconfig-file \fR[standard options ...]
.SH WARNING
...
<!--
    ...
    the file "tidy5-config.xml", created by running

        tidy5 -xml-config > tidy5-config.xml
...
.RS 4
.LP
https://github.com/htacg/tidy5/issues/
.RE
.LP
...
.LP
The sources for \fBHTML Tidy\fR are available at https://github.com/htacg/tidy5/ under the MIT Licence.
</xsl:template>
